...
class MultiTierCacheManager:
    ...
    def __init__(self, l1_capacity: int = 1000, l2_capacity: int = 10000):
        ...
        logger.info("MultiTierCacheManager initialized with L1_cap=%d and L2_cap=%d", l1_capacity, l2_capacity)
    ...
    def _evict_l1(self):
        if self.min_freq in self.freq_map:
            ...
            if evicted_node:
                ...
                logger.debug("Evicted key %s from L1 cache (freq: %d)", evicted_node.key, evicted_node.freq)

    def _background_sync(self):
        """Background worker to synchronize L1 cache writes to L2 persistent storage."""
    ...
    def shutdown(self):
        ...
        logger.info("MultiTierCacheManager gracefully shut down.")

# End of cache manager module.
...
# Example Usage:
# 
# cache = MultiTierCacheManager(l1_capacity=500, l2_capacity=5000)
# 
# # Putting items
# cache.put("user_profile_123", {"name": "Alice", "role": "Admin"})
# cache.put("config_settings", {"theme": "dark", "notifications": True})
# 
# # Getting items
# user = cache.get("user_profile_123")
# if user:
#     print(user["name"])
# 
# # Invalidating
# cache.invalidate("config_settings")
# 
# # Fetching stats
...
# cache.shutdown()
# 
# Advanced features of this cache manager:
...
#
# Ensuring reliable cache consistency across the cluster is critical, hence the
...
# the public API of the MultiTierCacheManager.
